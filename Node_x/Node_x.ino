...
void loop() {
  ...
  if(sensor == 0 && stateku == 0){
    stateku = 1;
  }
  if(sensor == 1 && stateku == 1){
   stateku = 0;
   akumulasi ++; 
  }
  Serial.print("nilai sensor : ");
  Serial.println(sensor);
  Serial.print("nilai state : ");
  Serial.println(stateku);
  Serial.print("Nilai Akumulator : ");
  Serial.println(akumulasi);
  
  //ukuran Form
  display.setFont(ArialMT_Plain_10);
  //colom, baris, isitext 
  display.drawString(0, 0, "Nabil Try to connected!");
  display.display();

  display.setFont(ArialMT_Plain_10);
  display.drawString(0, 14, "Water Meter Value:");
  display.display();

  display.setTextAlignment(TEXT_ALIGN_LEFT);
  display.setFont(ArialMT_Plain_10);
  display.drawString(0, 28, "Sending packet : " + String(counter));
  display.display();
